...
## 0-DATA-v10. 성심교정 중심 전체 기능 데모 데이터

- 자동 테스트: **23개 파일, 72개 테스트 통과**
- production/PWA build: 성공, 메인 JS 495.07kB(gzip 139.63kB)
- 위치 구성: 가톨릭대학교 성심교정 중심 좌표와 반경 500m 게시글 12개·빈 핀 4개
- 전국 구성: 부산·제주·서울·대전·광주·대구·강릉·전주 게시글 8개
- 기능 범위: 9개 카테고리, 댓글·한 단계 답글·5종 반응·신고 누적 숨김·반경 채팅
- 사진: 실존 로고·식별 가능한 인물 없이 제작한 캠퍼스·역곡 상권·부산·제주 사진 4장, 게시글 7개에 배치
- 상호작용: 글·핀·댓글·채팅 생성, 글 수정·삭제, 확인·추천·신고, 이미지 미리보기를 인메모리 저장소에서 지원
- 인증: 실제 회원가입·세션·프로필 조회 없이 `songsim_demo` 로컬 샘플 사용자로 진입
- 안전 경계: 개발 모드에서 명시적으로 켠 경우만 활성화되며 원격 Supabase에는 읽기·쓰기를 하지 않음
- 판정: **로컬 기능 시연·사용성 테스트용 GO. 새로고침 시 초기 샘플 상태로 복원됨**
...
- 설계 기준: 스위스 공공정보·한국형 편집·친근한 모바일·전문 공간정보·접근성 미니멀이라는 서로 다른 전문 분야로 화면 골격부터 분리
- 서브에이전트 검토: 서로 다른 성향의 독립 디자이너 5명이 작성한 브리프를 각각 반영
- 범위: 다섯 시안의 메뉴와 주요 목적지, 커뮤니티, 지도, 채팅 화면을 실제 브라우저에서 확인
- 기능 보존: 지도·커뮤니티·채팅·메뉴 탭, 글쓰기, 위치 보호, 기존 `02 골목 게시판` 복귀 경로 유지
- 반응형: 320×568에서 5개 시안 모두 `body.scrollWidth === innerWidth`, 선택 시안 자동 노출 확인
- 상태 유지: 선택한 시안을 localStorage에 저장하고 알 수 없는 값은 기본 시안으로 복구
- 자동 테스트: **19개 파일, 59개 테스트 통과**
- production/PWA build: 성공
- 판정: **5종 모두 비교 가능한 프로토타입으로 GO. 최종 선택 뒤 미사용 CSS를 제거해 번들 크기를 줄일 것**

## 0-UI-v7. 디자이너 리비전 결과

- 디자인 콘셉트: **동네 관찰 지도 + 골목 게시판**
- 자동 테스트: **18개 파일, 54개 테스트 통과**
- 프로덕션 빌드: Vite/PWA 빌드 성공
- 실제 브라우저: 기본 화면과 320×568에서 지도·커뮤니티·건물 목록·채팅·메뉴 탐색 확인
- 제거: 청록 그라데이션, 과도한 둥근 카드·그림자·컬러 아이콘 타일·pill 상태값, 모든 탭의 플로팅 글쓰기 버튼
- 추가: 미색/먹색/적갈색 팔레트, 도면선 기반 지도 계기판, 선형 게시판 목록, 문맥형 글쓰기 진입점, 현장 로그형 채팅
- 기능 보존: 지도·카테고리·핀·커뮤니티·채팅·알림·복구 코드·위치 보호·빠른 글쓰기
- 판정: **기존 v6보다 제품 고유성이 명확함. 실기기 사용자 선호 테스트 전까지 CONDITIONAL GO**

## 0-UI. kimiv v6 UI 업그레이드 결과

- UI 준비도: **9.0/10 — 기능 보존 브라우저 검증 완료, 다기기 실사용 베타 전까지 CONDITIONAL GO**
- 자동 테스트: **17개 파일, 53개 테스트 통과**
- 프로덕션 빌드: Vite/PWA 빌드 성공
- 실제 브라우저: 기본 화면과 320×568에서 지도·커뮤니티·건물 피드·채팅·메뉴·복구 코드 이동과 주요 인터랙션 확인
- 개선: 공통 아이콘·색상·카드·탐색 시스템, 메뉴 정보 구조, 지도 컨트롤 배치, 빈 상태 CTA, 채팅 연결 상태
- 보존 확인: 전체/위치별 커뮤니티, 알림 설정, 복구 코드, 위치 보호, 카테고리 필터, 빠른 글쓰기
- 남은 검증: iPhone·Android 실기기, 큰 글자 200%, 느린 네트워크, 5명 이상 비개발 사용자 관찰 테스트
