...
| report_count | int | default 0. report_post_v2 RPC가 누적 신고 수로 갱신 |
| hidden | boolean | default false. report_count가 5 이상이면 report_post_v2 RPC가 자동으로 true |
...
| hidden | boolean | default false. 5회 이상 신고되면 report_comment_v2 RPC가 자동으로 true |
| parent_comment_id | uuid | nullable. comments(id) FK, on delete cascade. 있으면 그 댓글의 답글(한 단계만, 답글의
답글은 없음) — 25번 참고 |
| reactions | jsonb | default '{}'. {emoji: count} 집계 캐시. react_to_comment_v2 RPC가 갱신 |
...
| reactor_hash | bytea | device secret의 SHA-256 hash. plaintext는 저장하지 않음 |
| created_at | timestamptz | default now() |

unique(comment_id, emoji, reactor_hash)가 있어 같은 브라우저가 같은 댓글에 같은 이모지를
다시 누르면 행이 지워진다(토글). RLS는 켜져 있지만 정책이 없어 anon/authenticated는 절대
직접 접근 못 한다 — `react_to_comment_v2` SECURITY DEFINER 함수로만 조작되고, 이 함수가
...
| owner_secret | text | 호환 컬럼. migration 후 항상 null |
| owner_secret_hash | bytea | 소유권 검사용 SHA-256 hash |
| device_secret_hash | bytea | nullable. 복구 코드 검색용 SHA-256 hash |
| recovery_ciphertext | bytea | nullable. owner secret을 복구 코드로 AES-256 암호화한 값 |

RLS는 켜져 있지만 정책이 하나도 없어서 anon/authenticated는 이 테이블에 절대
직접 접근 못 한다 — `create_post_v3`, `update_own_post_v2`, `delete_own_post_v2`,
`restore_ownership_v2` SECURITY DEFINER 함수로만 조작된다. **posts 테이블에 직접
쓰기 코드를 새로 만들지 말고 반드시 이 RPC를 통해서만 조작할 것** — 이게 "작성자만 자기 글 관리"의
핵심 보안 장치다 (진짜 로그인 없이도 안전하게 소유권을 확인하는 방식).

posts/comments/pins의 직접 insert/update/delete는 anon/authenticated에서 차단되어 있다.
공개 조회는 가능하지만 hidden=true인 글·댓글은 RLS 정책에서도 제외된다.
...
없고 `create_pin_v3`/`delete_own_pin_v2` RPC로만 조작한다(post_owners와
동일한 패턴). realtime publication에도 추가되어 있다. 여기에 더해
`delete_expired_pins_v2()` RPC가 있다 — 소유자
...
| reporter_hash | bytea | reporter secret의 SHA-256 hash. plaintext는 저장하지 않음 |
| created_at | timestamptz | default now() |

각각 unique(post_id/comment_id, reporter_hash) 제약이 있어 같은 브라우저가 같은 대상을
여러 번 신고해도 1회만 집계된다. RLS는 켜져 있지만 정책이 없어 anon/authenticated는
절대 직접 select/insert 못 한다 — `report_post_v2`/`report_comment_v2` SECURITY DEFINER
...
chat_messages의 direct SELECT/INSERT/UPDATE/DELETE는 차단되어 정확한 좌표를 전역으로
내려받을 수 없다. `get_nearby_chat_messages_v2`가 서버에서 500m/1km/2km 반경을 먼저
검사한 최대 200개만 반환하고, `send_chat_message_v2`가 입력검증·rate limit 후 등록한다.

### push_subscriptions 테이블 (웹 푸시 구독 — 관심 지역/키워드/조용한 시간, 회원가입 없음)
| 컬럼 | 타입 | 비고 |
|---|---|---|
| device_hash | bytea | pk/unique. device secret의 SHA-256 hash. plaintext는 저장하지 않음 |
...
RLS 활성화 + 정책 없음(anon/authenticated 직접 접근 불가) — `upsert_push_subscription_v2`/
`delete_push_subscription_v2` SECURITY DEFINER 함수로만 조작된다. 실제 발송은 배포 환경의
service-role worker가 담당하며 애플리케이션 migration은 구독 저장 계약까지만 제공한다.
...
21. 익명 글 시스템 신뢰 장치(신고/자동숨김/사건사고 강조/만료 로직 점검) — 완료
    ...
    - 모든 게시글(PostCard, PostDetail)과 모든 댓글(PostDetail)에 🚩 신고
      버튼이 붙는다. 신고는 로그인 없이 report_post_v2/report_comment_v2 RPC로
      기록되고(위 데이터 모델 섹션 참고), 같은 브라우저가 같은 대상을 여러
      번 눌러도 서버에서 1회만 집계된다(reporter_hash unique 제약). 신고
      ...
      트레이드오프).
    - 신고 누적 5회부터는 report_post_v2/report_comment_v2 RPC가 관리자 개입 없이
...
24. 회원가입 없는 익명 소유권 복구(복구 코드) — 완료 여부는
    src/components/RecoveryCode.jsx와 create_post_v3/create_pin_v3/
    restore_ownership_v2 RPC로 확인한다.
    - notifications.js의 기기별 32자 이상 secret을 하나의 복구 코드로 재사용한다.
      서버에는 device/owner secret plaintext를 저장하지 않는다. owner secret의 SHA-256
      hash는 권한 확인에, device hash는 복구 대상 검색에 사용하고, 복구용 owner secret은
      복구 코드로 AES-256 암호화한 recovery_ciphertext로만 보관한다.
    - restore_ownership_v2는 actor와 복구 코드 양쪽에 rate limit을 적용하고 최대 500개만
      복호화해 반환한다. post_owners/pin_owners 직접 조회 코드는 만들지 말 것.
...
25. 동네 채팅 반경 조절 + 댓글 이모지 반응/답글 — 완료 여부는
    ...
    - ChatRoom.jsx: 기존엔 NEARBY_RADIUS_METERS=1000 고정이었다. 이제
      500m/1km/2km 칩으로 고를 수 있다(기본 1km). 반경을 바꿀 때마다
      get_nearby_chat_messages_v2에 해당 반경을 전달해 서버에서 먼저 필터링하므로,
      클라이언트가 반경 밖의 정확한 채팅 좌표를 내려받지 않는다.
    ...
    - 댓글 이모지 반응: categories.js의 COMMENT_REACTION_EMOJIS(5종
      고정 팔레트) 중 하나를 누르면 react_to_comment_v2 RPC가 토글한다
      ...
      **새 realtime 채널을 만들지 않고** 기존 subscribeToComments의
      ...
      반영된다. reactor hash의 입력은 notifications.js의
    ...
    - "내가 이 반응을 이미 눌렀는지" 하이라이트 표시는 Comment.jsx
      ...
      누르면 정상적으로 토글된다, 서버 쪽 중복 방지는 reactor_hash
